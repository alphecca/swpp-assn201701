...
def signInVerification(driver, uname, upwd):
    ...
    driver.find_element_by_id('username_field').send_keys("chatA")
    driver.find_element_by_id('password_field').send_keys("chatA")
...
def B_sendTextVerification(driver, link, uname, upwd ):
    try:
       res = requests.get(link+"chatroom/1/user/", auth=(uname, upwd))
       if res.status_code != 200:
         print("ERROR: Cannot get {0} : {1}, id = {2}, pwd= {3}".format(link+"chatroom/1/user/", res.status_code, uname, upwd))
         exit(1)
    except Exception:
        print("ERROR: Cannot get {0}".format(link))
        sleep(1)
